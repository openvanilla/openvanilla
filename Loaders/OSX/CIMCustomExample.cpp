...
{
	char buf[256];
	sprintf (buf, "IM global addr=%x", &globalcounter);
    InsertMenuItemTextWithCFString(mnu, 
        CFSTR("Print total chars typed to stderr"), 0, 0, 'TOTL');
    InsertMenuItemTextWithCFString(mnu, 
        CFStringCreateWithCString(NULL, buf, kCFStringEncodingUTF8), 0, 0, 'TOTL');
...
int CIMCustomMenuHandler(void *data, UInt32 command, MenuRef mnu, CIMInputBuffer *buf)
{
    CustomCounter *cntr=(CustomCounter*)data;
    switch (command)
    {
