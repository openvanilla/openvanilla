...
protected:
    ...
    VXPreference *pref;
    VXMenu *menu;
    VXInfoBoxController *candi, *notifi;
    NSArray *modules;
};

int VXLoader::init(MenuRef m, NSString *prefFile)
{
    candi=[[VXInfoBoxController alloc] initWithWindowNibName: @"VXLoader"];
    notifi=[[VXInfoBoxController alloc] initWithWindowNibName: @"VXLoader"];    
    modules=[[NSArray alloc] init];
    if (!candi || !notifi || modules) return 0;
    
    service=new VXService;
    pref=new VXPreference(prefFile);
    menu=new VXMenu(this, m);
    if (!service || !pref || !menu) return 0;    
    
    stamp=pref->timeStamp();
    
    VXDictionary globalPref(pref->getLoaderPrefRoot());
    [modules addObjectsFromArray: 
        VXLoadEveryThing([pref->getLoaderPrefKey(@"modulePathes", &gp, srv)];
    if (![modules count]) return 0;
