...
- (void)fillRect:(NSRect)rect angle:(float)angle radius:(float)radius
  ...
  CGContextSaveGState(currentContext);
	  ...
	  float fw, fh;	
	  float ovalWidth = radius;
	  float ovalHeight = radius;	  
	  CGContextBeginPath (currentContext);	

	  CGContextTranslateCTM (currentContext, NSMinX(rect), NSMinY(rect));
	  CGContextScaleCTM (currentContext, ovalWidth, ovalHeight);
	  fw = NSWidth (rect) / ovalWidth;
	  fh = NSHeight (rect) / ovalHeight;
				  
	  CGContextMoveToPoint(currentContext, fw, fh/2); 
	  CGContextAddArcToPoint(currentContext, fw, fh, fw/2, fh, 1);
	  CGContextAddArcToPoint(currentContext, 0, fh, 0, fh/2, 1);
	  CGContextAddArcToPoint(currentContext, 0, 0, fw/2, 0, 1);
	  CGContextAddArcToPoint(currentContext, fw, 0, fw, fh/2, 1); 
