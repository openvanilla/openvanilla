...
extern "C" {
	...
	int OVInitializeLibrary(OVService *s, const char *libpath);
...
void MLInitialize(int moduleID)
{
	...
	if (!OVInitializeLibrary(g_service, "/var/root/")) {
		murmur("failed to init input method module library");
		abort();
	}

    g_inputMethod = (OVInputMethod*)OVGetModuleFromLibrary(moduleID);
    if (!g_inputMethod) {
