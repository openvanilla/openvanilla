// OVIMPOJHakka.cpp: prototype for POJHakka IM

// Copyright (c) 2004 by Lukhnos D. Liu, Kang-ming Liu, and Pek-tiong Tan

#define OVDEBUG 1
#include "OVIMPOJHakka.h"

OVLOADABLEWRAPPER(OVIMPOJHakka);
...
int OVIMPOJHakka::identifier(char* s)
{
    return strlen(strcpy(s, "OVIMPOJHakka"));
}

int OVIMPOJHakka::name(char *locale, void *s, OVEncoding *enc)
{
    *enc=ovEncodingUTF8;
    ...
    OVService* srv, char* path)
{
    ...
    strcpy(cinpath, path);
    strcat(cinpath, "OVIMPOJHakka/pojhakka.cin");        
    murmur ("OVIMPOJHakka: loading pojhakka.cin at %s", cinpath);
...
int OVIMPOJHakka::terminate(OVDictionary*, OVDictionary*, OVService*)
{
    if (cintab) delete cintab;
    return 1;
}

int OVIMPOJHakka::update(OVDictionary* global, OVDictionary* local)
{
...
OVIMContext *OVIMPOJHakka::newContext()
{
    return new OVIMPOJHakkaContext(this, cintab);
}

OVIMPOJHakkaContext::OVIMPOJHakkaContext(OVIMPOJHakka *p, VXCIN *ctab)
{
    parent=p;
    cintab=ctab;
...
OVIMPOJHakkaContext::~OVIMPOJHakkaContext()
{
    [list release];
}
...
int OVIMPOJHakkaContext::candidateEvent(OVKeyCode *key, OVBuffer *buf, 
    OVTextBar *textbar, OVService *srv)
{
