...
ComponentResult CIMSessionActivate(CIMSessionHandle hndl)
{
	...
	#ifdef CIMCUSTOM
		if (CIMCustomActivate((*hndl)->data)) result=noErr;
		else result=invalidHandler;
	#endif
...
ComponentResult CIMSessionDeactivate(CIMSessionHandle hndl)
{
	...
	#ifdef CIMCUSTOM
		if (!CIMCustomDeactivate((*hndl)->data)) return invalidHandler;
	#endif
...
ComponentResult CIMSessionEvent(CIMSessionHandle hndl, EventRef evnt)
{
    ...
    GetEventParameter(evnt, kEventParamKeyModifiers, typeUInt32, nil,
		...
		return CIMCustomHandleInput((*hndl)->buffer->bind((*hndl)->instance),
			charcode, keycode, modifiers, &p, (*hndl)->data);
	#else
    ...
    EventRef evnt, void *userdata)
{
	...
	#ifdef CIMCUSTOM
		if (CIMCustomMenuHandler(command.commandID, cimPencilMenu, (*cimActiveSession)->data)) 
			return noErr;
	#else
